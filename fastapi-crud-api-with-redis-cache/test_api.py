...
@pytest.mark.parametrize("user_id", [faker.uuid4()])
async def test_delete_unknown_user(client: AsyncClient, user_id: str) -> None:
...
@pytest.mark.parametrize("user_id", [faker.uuid4()])
async def test_update_unknown_user(client: AsyncClient, user_id: str) -> None:
...
async def test_get_users(client: AsyncClient, total_users: int) -> None:
...
@pytest.mark.parametrize("total_users", [faker.random_int(min=10, max=50)])
async def test_get_users_limit(client: AsyncClient, total_users: int) -> None:
  # Create random N users
  await create_n_users_asynchronously(client, total_users)
...
async def test_get_users_limit_with_offset(
  ...
  request_filters = UserSelectFilters(
    limit=total_users_to_fetch,
    offset=offset,
  )
  resp = await client.get(
    "/users",
